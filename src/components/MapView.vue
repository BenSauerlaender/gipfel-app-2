<template>
  <div id="placeholdermap" class="mapContainer"></div>
...
const summitsAscendedIDs = computed(() =>
  dataStore.summits
      ...
      if (leafletMap.value) {
        ...
        marker.on('popupopen', function (e) {
          const popupEl = document.querySelector(
            '.leaflet-popup-link[data-route="/routes/' + summit._id + '"]',
          )
          if (popupEl) {
            popupEl.onclick = (evt) => {
              evt.preventDefault()
