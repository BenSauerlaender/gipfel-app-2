...
export const useDataStore = defineStore('data', {
  ...
  getters: {
    ...
    f_Ascents(state) {
        ...
        .filter(ascent => {
            ...
            if(filterStore.filters.grade.normal.min !== null && grade < filterStore.filters.grade.normal.min) return false
            if(filterStore.filters.grade.normal.max !== null && grade > filterStore.filters.grade.normal.max) return false
            return true
        })
        .filter(ascent => {
  ...
  actions: {
    ...
    async loadData() {
      ...
      // Load data from API
      const [ascentsResponse, climbersResponse, routesResponse, summitsResponse, regionsResponse, tripsResponse] = await Promise.all([
        getAscents(),
        getClimbers(),
        getRoutes(),
        getSummits(),
